use crate::message::{Message, Role, ReasoningDetail, ToolCall, ToolCallFunction, ToolDef};
...
impl Event {
    pub fn text_delta(delta: &str) -> Self {
        Event { event_type: "text_delta".into(), delta: delta.into(), ..Default::default() }
    }
    pub fn reasoning_delta(delta: &str) -> Self {
        Event { event_type: "reasoning_delta".into(), delta: delta.into(), ..Default::default() }
    }
    pub fn tool_call_start(tc: &StreamToolCall) -> Self {
        Event {
            event_type: "tool_call_start".into(),
            tool_call_id: tc.id.clone(), tool_call_name: tc.name.clone(),
            tool_call_args: tc.arguments.clone(), ..Default::default()
        }
    }
    pub fn tool_result_ev(tc: &StreamToolCall, result: &str, duration: &str) -> Self {
        Event {
            ...
            tool_call_id: tc.id.clone(), tool_call_name: tc.name.clone(),
            tool_call_args: tc.arguments.clone(), tool_result: result.into(),
            tool_duration: duration.into(), ..Default::default()
        }
    }
    pub fn tool_error_ev(tc: &StreamToolCall, msg: &str, duration: &str) -> Self {
        Event {
            ...
            tool_call_id: tc.id.clone(), tool_call_name: tc.name.clone(),
            tool_error: msg.into(), tool_duration: duration.into(), ..Default::default()
        }
    }
    pub fn tool_error_deferred(tc: &StreamToolCall, msg: &str) -> Self {
        Event {
            event_type: "tool_error".into(),
            tool_call_id: tc.id.clone(), tool_call_name: tc.name.clone(),
            tool_error: msg.into(), ..Default::default()
        }
    }
    pub fn error_msg(msg: &str) -> Self {
        Event { event_type: "error".into(), error: msg.into(), ..Default::default() }
    }
    pub fn retry_ev(msg: &str) -> Self {
        Event { event_type: "retry".into(), error: msg.into(), ..Default::default() }
    }
    pub fn retry_available(msg: &str) -> Self {
        Event { event_type: "retry_available".into(), error: msg.into(), ..Default::default() }
    }
    pub fn agent_done(finish_reason: &str) -> Self {
        Event { event_type: "agent_done".into(), finish_reason: finish_reason.into(), ..Default::default() }
    }
    pub fn usage_ev(usage: Usage) -> Self {
        Event { event_type: "usage".into(), usage: Some(usage), ..Default::default() }
    }
}
...
pub fn build_system_prompt(system_md: &str, global_md: &str, local_md: &str, system_prefix: &str) -> String {
    let mut sb = String::new();
    if !system_md.is_empty() { sb.push_str(system_md); sb.push_str("\n\n"); }
    if !system_prefix.is_empty() { sb.push_str(system_prefix); sb.push_str("\n\n"); }
    sb.push_str(TOOL_RULES_PROMPT);
    if !global_md.is_empty() { sb.push_str("\n\n## User conventions\n"); sb.push_str(global_md); }
    if !local_md.is_empty() { sb.push_str("\n\n## Project conventions\n"); sb.push_str(local_md); }
    sb
}
...
impl AgentSession {
    ...
    pub fn sess(&self) -> Session { self.sess.clone() }
    ...
    pub fn system_prompt(&self) -> &str { &self.system_msg }
    pub fn context_window(&self) -> i32 { self.client.context_window() }
    pub fn context_tokens(&self) -> i32 { self.last_total_tokens }
    pub fn tool_defs(&self) -> Vec<ToolDef> { self.cached_tool_defs.clone() }
    ...
    pub fn set_subagents(&mut self, defs: HashMap<String, SubagentDef>) {
        ...
        names.sort();
        let descriptions: HashMap<String, String> = defs.iter()
            .map(|(k, v)| (k.clone(), v.description.clone())).collect();
        self.subagents = defs;
        self.cached_tool_defs.push(build_delegate_def(&names, &descriptions));
    }
    ...
    pub fn prompt(&mut self, user_text: &str) -> mpsc::Receiver<Event> {
        ...
        tokio::spawn(async move { s.run_loop(&ut, &tx).await; });
        rx
    }

    pub fn retry(&self) -> Result<mpsc::Receiver<Event>, String> {
        if self.last_prompt.is_empty() { return Err("no prompt to retry".into()); }
        let (tx, rx) = mpsc::channel(100);
        let mut s = self.clone();
    ...
    pub fn compact(&self) -> Result<mpsc::Receiver<Event>, String> {
        ...
        tokio::spawn(async move {
            ...
            let turns = {
                ...
                store.ancestry(&s.sess.id, &s.sess.current_turn)
    ...
    fn clone(&self) -> Self {
        AgentSession {
            store: self.store.clone(), sess: self.sess.clone(), tools: self.tools.clone(),
            client: self.client.clone(), system_msg: self.system_msg.clone(),
            max_rounds: self.max_rounds, cwd: self.cwd.clone(),
            total_prompt_tokens: self.total_prompt_tokens,
            total_completion_tokens: self.total_completion_tokens,
            total_cost: self.total_cost, last_request_tokens: self.last_request_tokens,
            ...
            subagents: self.subagents.clone(), subagent_turns: self.subagent_turns.clone(),
            is_subagent: self.is_subagent, last_prompt: self.last_prompt.clone(),
    ...
    async fn run_loop(&mut self, user_text: &str, events: &mpsc::Sender<Event>) {
        ...
        self.working_messages.push(Message {
            role: Role::User, content: user_text.into(),
            reasoning_content: String::new(), reasoning_details: vec![],
            tool_calls: vec![], tool_call_id: String::new(), name: String::new(),
        ...
        let ancestry_msgs = {
            let turns_result = {
                ...
                store.ancestry(&self.sess.id, &parent_id)
                    .map_err(|e| e.to_string())
            };
            match turns_result {
                Ok(turns) => {
                    let ancestry_msgs = self.compact_ancestry(&turns, false).await;
                    log::debug!("agent phase phase=compact dur={:?} turns={}", t0.elapsed(), turns.len());
                    ancestry_msgs
                }
                Err(e) => {
                ...
                Ok(result) => {
                    ...
                    if !result.tool_calls.is_empty() {
                        if !result.content.is_empty() {
                            self.working_messages.push(Message {
                                role: Role::Assistant, content: result.content.clone(),
                                reasoning_content: result.reasoning.clone(),
                                reasoning_details: vec![], tool_calls: vec![],
                                tool_call_id: String::new(), name: String::new(),
                                tool_duration: String::new(),
                            });
                        }
                        let t0 = std::time::Instant::now();
                        let pending = self.execute_tools(&result.tool_calls, events).await;
                        log::debug!("agent phase phase=round round={} stream={:?} tools={:?} n_tools={}",
                            round, stream_dur, t0.elapsed(), result.tool_calls.len());
                        let msg_reasoning = if result.content.is_empty() { result.reasoning.clone() } else { String::new() };
                        self.append_tool_messages(&pending, &msg_reasoning, &result.reasoning_details);
                        continue;
                    }

                    if !result.content.is_empty() || !result.reasoning.is_empty() || !result.reasoning_details.is_empty() {
                        self.working_messages.push(Message {
                    ...
                    log::debug!("agent phase phase=commit dur={:?} total={:?}", t0.elapsed(), loop_start.elapsed());

                    let fr = if result.finish_reason.is_empty() { "stop" } else { &result.finish_reason };
    ...
    fn build_request(&self, ancestry_msgs: &[Message]) -> ChatRequest {
        ...
        msgs.push(Message {
            role: Role::System, content: self.system_msg.clone(),
            reasoning_content: String::new(), reasoning_details: vec![],
            tool_calls: vec![], tool_call_id: String::new(), name: String::new(),
            tool_duration: String::new(),
        });
        ...
        ChatRequest {
            model: String::new(), messages: msgs,
            tools: self.cached_tool_defs.clone(), stream: false, max_tokens: 0,
            stream_options: None, thinking: None, reasoning_effort: String::new(),
            reasoning: None, models: vec![], route: String::new(),
            provider_prefs: None, cache_control: None, session_id: self.sess.id.clone(),
        }
    }

    // ── stream_round ─────────────────────────────────────────────────────

    async fn stream_round(&mut self, req: ChatRequest, events: &mpsc::Sender<Event>) -> Result<RoundResult, ProviderError> {
        ...
        for attempt in 0..MAX_ATTEMPTS {
            match self.client.chat(req.clone()).await {
                ...
                Err(e) => {
                    if !e.retryable() { return Err(e); }
                    if attempt < MAX_ATTEMPTS - 1 {
                        let shift = 1u64 << attempt;
                        let delay = Duration::from_secs(2 * shift).min(MAX_DELAY);
                        let jitter = delay.as_secs_f64() * 0.25 * (2.0 * rand::thread_rng().gen::<f64>() - 1.0);
                        let delay = Duration::from_secs_f64(delay.as_secs_f64() + jitter);
                        let _ = events.send(Event::retry_ev(&format!(
                            "Attempt {}/{} failed: {} — retrying in {}",
                            attempt + 1, MAX_ATTEMPTS, e, format_duration(delay)
                        ))).await;
        ...
        let mut rx = rx.ok_or_else(|| ProviderError { status_code: 0, body: "chat returned no stream".into() })?;
        let mut result = RoundResult {
            content: String::new(), reasoning: String::new(),
            reasoning_details: vec![], tool_calls: vec![], finish_reason: String::new(),
        };

        while let Some(ev) = rx.recv().await {
            if let Some(ref err) = ev.error { return Err(err.clone()); }
            ...
            match ev.event_type.as_str() {
                ...
                "reasoning_delta" => {
                    ...
                    let _ = events.send(Event::reasoning_delta(&ev.reasoning_delta)).await;
                }
                "tool_call" => {
                    if let Some(ref tc) = ev.tool_call { result.tool_calls.push(tc.clone()); }
                }
                "usage" => {
                ...
                "reasoning_details" => { result.reasoning_details = ev.reasoning_details.clone(); }
                "finish_reason" => { result.finish_reason = ev.finish_reason.clone(); }
                _ => {}
            }
        }
        Ok(result)
    ...
    async fn execute_tools(&mut self, tool_calls: &[StreamToolCall], events: &mpsc::Sender<Event>) -> Vec<PendingTool> {
        ...
        for (i, tc) in tool_calls.iter().enumerate() {
            ...
            if tc.name == "delegate" && !self.subagents.is_empty() {
                ...
                set.spawn(async move {
                    ...
                    let (result, dd) = run_delegate(&tc, &subagents, &store, &sess_id, max_rounds, &cwd, &events).await;
                    let dur = format_duration(start.elapsed());
                    let _ = events.send(Event::tool_result_ev(&tc, &result, &dur)).await;
                ...
                set.spawn(async move {
                    ...
                    match tools.get(&tc.name) {
                        ...
                        Some(tool) => {
                            ...
                            let tool_result = tokio::time::timeout(Duration::from_secs(120), (tool.execute)(args)).await;
                            let dur = format_duration(start.elapsed());
                            match tool_result {
                                Ok(Ok(result)) => {
                                    ...
                                    let _ = events.send(Event::tool_result_ev(&tc, &final_result, &dur)).await;
        ...
        // Collect results
        let mut pending: Vec<PendingTool> = (0..n).map(|i| PendingTool {
        ...
        while let Some(result) = set.join_next().await {
            if let Ok((i, result_str, dur, is_error, delegate_data)) = result {
                ...
                if is_error {
                    deferred_errors.push(Event::tool_error_ev(&pending[i].call, &pending[i].result, &pending[i].duration));
                }
    ...
    fn append_tool_messages(&mut self, pending: &[PendingTool], reasoning: &str, details: &[ReasoningDetail]) {
        let assistant_tool_calls: Vec<ToolCall> = pending.iter().map(|pt| ToolCall {
            id: pt.call.id.clone(), call_type: "function".into(),
            function: ToolCallFunction { name: pt.call.name.clone(), arguments: pt.call.arguments.clone() },
        }).collect();

        self.working_messages.push(Message {
            role: Role::Assistant, content: String::new(),
            reasoning_content: reasoning.into(), reasoning_details: details.to_vec(),
            tool_calls: assistant_tool_calls,
            tool_call_id: String::new(), name: String::new(), tool_duration: String::new(),
        });

        for pt in pending {
            let content = if pt.result.is_empty() { "(no output)".into() } else { pt.result.clone() };
            self.working_messages.push(Message {
                role: Role::Tool, content,
                reasoning_content: String::new(), reasoning_details: vec![],
                tool_calls: vec![], tool_call_id: pt.call.id.clone(),
                name: pt.call.name.clone(), tool_duration: pt.duration.clone(),
            });
        }
    }
    ...
    async fn commit_turn(&mut self, parent_id: &str) {
        ...
        let turn = Turn {
            id: turn_id.clone(), parent_id: parent_id.into(),
            messages: self.working_messages.clone(), created_at: Utc::now(),
            subagent: String::new(), tool_call_id: String::new(),
        };

        {
            ...
            for st in &self.subagent_turns {
                ...
                let sub_turn = Turn {
                    id: sub_id, parent_id: turn_id.clone(),
                    messages: st.msgs.clone(), created_at: Utc::now(),
                    subagent: st.subagent.clone(), tool_call_id: st.tool_call_id.clone(),
                };
    ...
    async fn compact_ancestry(&mut self, turns: &[Turn], force: bool) -> Vec<Message> {
        ...
        if self.compacted_summary.is_empty() {
            if !above_threshold { return flatten_turns(turns); }
            if turns.len() <= force_keep { return flatten_turns(turns); }
        ...
        match call_compaction(compaction_client, &build_incremental_summary_prompt(&self.compacted_summary, summary_turns)).await {
            Ok(summary) => {
                self.compacted_summary = summary.clone();
...
fn flatten_turns(turns: &[Turn]) -> Vec<Message> {
    ...
    for t in turns { msgs.extend_from_slice(&t.messages); }
    msgs
}
...
fn build_compacted_messages(summary: &str, recent_msgs: &[Message]) -> Vec<Message> {
    ...
    out.push(Message {
        role: Role::System, content: summary.into(),
        reasoning_content: String::new(), reasoning_details: vec![],
        tool_calls: vec![], tool_call_id: String::new(), name: String::new(),
        tool_duration: String::new(),
    });
...
fn compute_keep_turns(turns: &[Turn], context_window: i32) -> usize {
    ...
    const FLOOR: usize = 3;
    if kept < FLOOR { kept = FLOOR; }
    if kept > turns.len() { kept = turns.len(); }
    kept
}
...
async fn run_delegate(
    ...
    #[derive(Deserialize)]
    struct DelegateParams { subagent: String, task: String, #[serde(default)] context: String }

    let params: DelegateParams = match serde_json::from_str(&tc.arguments) {
        Ok(p) => p,
        Err(e) => return (format!("invalid delegate params: {}", e), DelegateData {
            prompt_tokens: 0, completion_tokens: 0, cost: 0.0,
            msgs: vec![], subagent_id: String::new(), tool_call_id: tc.id.clone(),
        }),
    };

    let def = match subagents.get(&params.subagent) {
        Some(d) => d,
        None => return (format!("subagent {:?} not found", params.subagent), DelegateData {
            prompt_tokens: 0, completion_tokens: 0, cost: 0.0,
            msgs: vec![], subagent_id: String::new(), tool_call_id: tc.id.clone(),
        }),
    };
    ...
    if !params.context.is_empty() { task_text.push_str("\n\n"); task_text.push_str(&params.context); }
    ...
    let mut sub = AgentSession {
        store: store.clone(),
        sess: Session {
            id: sess_id.to_string(), name: String::new(), hash: String::new(),
            named: false, current_turn: String::new(), created_at: Utc::now(),
            updated_at: Utc::now(), turn_count: 0, prompt_tokens: 0,
            completion_tokens: 0, total_tokens: 0, context_tokens: 0,
            cost: 0.0, compacted_summary: String::new(), compacted_up_to: String::new(),
        },
        ...
        last_request_tokens: 0, last_total_tokens: 0,
        working_messages: Vec::new(),
        cached_ancestry_msgs: Vec::new(), cached_ancestry_turn_id: String::new(),
        subagents: HashMap::new(), subagent_turns: Vec::new(),
        last_prompt: String::new(), captured_msgs: Vec::new(),
        compaction_client: None, compacting: false, compacted_summary: String::new(), compacted_up_to: String::new(),
    };

    let (event_tx, mut event_rx) = mpsc::channel(100);
    let sub_id = def.id.clone();
    let tc_id = tc.id.clone();
    let parent_tx = parent_events.clone();

    let (result_tx, result_rx) = std::sync::mpsc::channel();

    let handle = std::thread::spawn(move || {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(sub.run_loop(&task_text, &event_tx));
        let _ = result_tx.send((sub.total_prompt_tokens, sub.total_completion_tokens, sub.total_cost, sub.captured_msgs, sub.working_messages));
    });
    ...
    let _ = handle.join();
    let (prompt_tokens, completion_tokens, cost, captured_msgs, working_msgs) = match result_rx.recv() {
        Ok(data) => data,
        Err(_) => {
            return ("(subagent encountered an error)".into(), DelegateData {
                prompt_tokens: 0, completion_tokens: 0, cost: 0.0,
                msgs: vec![], subagent_id: sub_id, tool_call_id: tc_id,
            });
        }
    };

    let msgs = if captured_msgs.is_empty() { working_msgs } else { captured_msgs };

    if had_error {
        return ("(subagent encountered an error)".into(), DelegateData {
            prompt_tokens, completion_tokens, cost,
            msgs, subagent_id: sub_id, tool_call_id: tc_id,
        });
    }

    let content = extract_final_content(&msgs);
    let final_result = if content.is_empty() { "(subagent produced no output)".into() } else { content };

    (final_result, DelegateData {
        prompt_tokens, completion_tokens, cost,
        msgs, subagent_id: sub_id, tool_call_id: tc_id,
    })
}
...
fn build_delegate_def(names: &[String], descriptions: &HashMap<String, String>) -> ToolDef {
    ...
    for (i, name) in names.iter().enumerate() {
        ...
        if let Some(d) = descriptions.get(name) {
            if !d.is_empty() { desc.push_str(": "); desc.push_str(d); }
        }
    }
    ...
    props.insert("subagent".into(), serde_json::json!({
...
async fn compact_web_fetch_result(client: &Arc<dyn ChatClient>, raw: &str) -> String {
    ...
    match call_compaction(client, &prompt).await {
        Ok(compacted) => {
            ...
            if compacted.is_empty() || PREFIX.len() + compacted.len() >= raw.len() { raw.into() }
            else { format!("{}{}", PREFIX, compacted) }
        }
        Err(_) => raw.into(),
    }
}
...
async fn call_compaction(client: &Arc<dyn ChatClient>, prompt: &str) -> Result<String, String> {
    let req = ChatRequest {
        model: String::new(), messages: vec![Message {
            role: Role::User, content: prompt.into(),
            reasoning_content: String::new(), reasoning_details: vec![],
            tool_calls: vec![], tool_call_id: String::new(), name: String::new(),
            tool_duration: String::new(),
        }],
        tools: vec![], stream: false, max_tokens: 0,
        stream_options: None, thinking: None, reasoning_effort: String::new(),
        reasoning: None, models: vec![], route: String::new(),
        provider_prefs: None, cache_control: None, session_id: String::new(),
    };

    let mut rx = client.chat(req).await.map_err(|e| e.to_string())?;
    let mut result = String::new();
    while let Some(ev) = rx.recv().await {
        if let Some(err) = ev.error { return Err(err.to_string()); }
        if ev.event_type == "text_delta" { result.push_str(&ev.delta); }
    }
    Ok(result)
}
...
fn format_turns_for_summary(sb: &mut String, turns: &[Turn]) {
    for turn in turns {
        for msg in &turn.messages {
            match msg.role {
                Role::User => { sb.push_str("User: "); sb.push_str(&msg.content); sb.push('\n'); }
                Role::Assistant => {
                    if !msg.content.is_empty() { sb.push_str("Assistant: "); sb.push_str(&msg.content); sb.push('\n'); }
                    if !msg.reasoning_content.is_empty() { sb.push_str("Assistant reasoning: "); sb.push_str(&msg.reasoning_content); sb.push('\n'); }
                    for tc in &msg.tool_calls {
                        sb.push_str("Tool call: "); sb.push_str(&tc.function.name);
                        sb.push('('); sb.push_str(&tc.function.arguments); sb.push_str(")\n");
                    }
                }
                Role::Tool => {
                    sb.push_str("Tool result ("); sb.push_str(&msg.name);
                    sb.push_str("): "); sb.push_str(&truncate_for_summary(&msg.content, 2000)); sb.push('\n');
                }
                Role::System => {}
            }
        }
...
mod tests {
    ...
    fn dummy_session() -> Session {
        Session {
            id: "s1".to_string(), name: String::new(), hash: "h".to_string(),
            named: false, current_turn: String::new(),
            created_at: chrono::Utc::now(), updated_at: chrono::Utc::now(),
            turn_count: 0, prompt_tokens: 0, completion_tokens: 0,
            total_tokens: 0, context_tokens: 0, cost: 0.0,
            compacted_summary: String::new(), compacted_up_to: String::new(),
        }
    }
    ...
    fn dummy_agent() -> AgentSession {
        ...
        let client: Arc<dyn ChatClient> = Arc::new(Client::new(
            "http://localhost", "m", "k",
            ModelProfile {
                context_window: 8000, max_output_tokens: 0,
                thinking_type: String::new(), reasoning_effort: String::new(),
                reasoning_max_tokens: 0, open_router: false,
                input_price: 0.0, cached_input_price: 0.0, output_price: 0.0,
                fallback_models: vec![], route: String::new(),
                provider_prefs: None, prompt_cache: false, prompt_cache_ttl: String::new(),
            }));
        let registry = Arc::new(Registry::new());
        AgentSession::new(store, dummy_session(), client, registry, "sys".to_string(), 5, "/tmp".to_string())
    }
