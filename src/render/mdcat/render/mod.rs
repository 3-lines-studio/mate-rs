...
use super::resources::ResourceUrlHandler;
use super::terminal::capabilities::TerminalCapabilities;
use super::theme::CombineStyle;
use super::{Environment, Settings, Theme};
...
pub(in crate::render::mdcat) mod highlighting;

use super::references::UrlBase;
use state::*;
use write::*;

use super::terminal::capabilities::StyleCapability;
use super::terminal::osc::{clear_link, set_link_url};
pub use data::StateData;
use data::{CurrentLine, CurrentTable};
use state::MarginControl::NoMargin;
    ...
    match (state, event) {
        ...
        (Stacked(stack, HighlightBlock(mut attrs)), Text(text)) => {
            for line in LinesWithEndings::from(&text) {
                ...
                match &settings.syntax_theme {
                    Some(theme) => {
                        let h = highlighting::highlighter_for(theme);
                        let regions =
        ...
        (
            ...
            let image_state = match (&settings.terminal_capabilities.image, resolved_link) {
                (Some(_), Some(ref url)) => {
                    log::warn!("Image rendering not supported: {url}");
                    None
                }
                (None, Some(url)) => {
                    if let InlineLink = state {
                        ...
                        log::info!(
                            "Terminal does not support images, rendering image as link ({url})"
                        );
            ...
            let (image_state, data) = match image_state {
                ...
                None => {
                    log::warn!("Rendering image {} as inline text, without link", dest_url);
